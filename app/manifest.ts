 ...
 * link their own `/<tenant>/manifest.webmanifest`.
 *
 * Where an origin carries several tenants, each tenant's manifest installs as a
 * separate app launching into that tenant; on an origin holding one tenant
 * reached by host, this manifest and the tenant's are the same app, because an
 * app is identified by the address it launches at and both launch at the root.
 *
