...
export async function pollPaymentRequestStatus({
  ...
  while (Date.now() < deadline) {
    ...
    if (!paymentContext) {
      console.warn('[HUBPISP][POLL] Payment context not found, stopping poll', {
        contextId,
      });
      return;
    }
    ...
    if (
      ...
      console.log('[HUBPISP][POLL] Context in terminal state, stopping poll', {
        contextId,
    ...
    try {
      ...
      statusReasonInformation = (paymentRequest?.creditTransferTransaction?.[0]
        ?.statusReasonInformation ||
        paymentRequest?.statusReasonInformation) as string | undefined;
    } catch (err) {
      console.error('[HUBPISP][POLL] Failed to fetch payment request status', {
        paymentRequestResourceId,
